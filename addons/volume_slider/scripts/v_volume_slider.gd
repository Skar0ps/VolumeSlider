...
@export_tool_button("Create a volume label", "Label") var _label_create: Callable:
	get(): return VolumeUtils.create_label.bind(self, bus_name)
...
func _ready() -> void:
	...
	_check_drag_ended_connection()
	VolumeUtils.sync_slider_with_bus(self, bus_name, save_volume)
...
func _on_drag_ended(value_changed: bool) -> void:
	if save_volume and value_changed:
		VolumeUtils.save_persisted_volume(bus_name, VolumeUtils.value_to_db(value))
...
func _validate_property(property: Dictionary) -> void:
	match property.name:
		...
		"round_display_volume":
			if rounded or not is_instance_valid(display_label):
