...
func _parse_property(
	...
	match name:
		"min_value":
			var property_control: Control = DB_SUFFIX_PROPERTY.new(false, true, false)
			property_control.spin.hide_slider = true
			property_control.spin.read_only = true
			add_property_editor(name, property_control)
			return true
		"max_value":
			add_property_editor(
					name,
					DB_SUFFIX_PROPERTY.new(false, false, false, true, 100.0, 200.0),
			)
			return true
		"value":
